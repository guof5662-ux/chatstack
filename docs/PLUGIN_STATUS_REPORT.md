...
| TOC | 已实现 | 前 36 字标题、点击跳转高亮、流式 debounce（platform-adapter 约 500ms） |
| 搜索 | 已实现 | 全文搜索、匹配片段、用户/助手消息均可跳转（TEST_REPORT 已记录修复） |
...
| 收藏 | 已实现 | 消息收藏、按收藏筛选 |
| 多平台 | 已实现 | ChatGPT、Gemini 两套 content_scripts；Claude 在 i18n/筛选中有占位 |
| 设置 | 已实现 | 自动保存、语言（zh/en/auto）、主题（light/dark/auto）、清空数据 |

### 已知局限（与 README/TEST_REPORT 一致）

- ChatGPT DOM 改版时，`chatgpt-adapter.js` 等选择器需调整。
- 自动项目多数归入「Inbox (Auto)」，依赖页面 DOM/URL。
- 无自动化测试，依赖手动清单（TEST_REPORT.md）。
...
| 类别 | 状态 | 说明 |
|------|------|------|
| 模块划分 | 达标 | content（入口、adapter、sidebar 子模块）/ core（storage、i18n、toc、project）清晰 |
| 依赖方向 | 达标 | sidebar 编排 export/toc/filters；adapter 与平台解耦；core 无 UI 依赖 |
| 单文件体量 | 部分达标 | `content/sidebar.js` 约 4085 行，体量偏大，建议后续按 Tab/导出/项目拆子模块 |
| 死代码 | 达标 | 此前已移除 getLayoutRoot()、isUserClosed() 等死代码 |
...
| 类别 | 状态 | 说明 |
|------|------|------|
| i18n | 达标 | core/i18n.js 中英文齐全，覆盖 header/tab/filter/action/empty/export/dialog 等；data-i18n / data-i18n-placeholder 等绑定 |
| 硬编码 | 达标 | 界面文案经 i18n；极少数占位（如「暂无其他项目」）可后续补 key |
...
| 类别 | 状态 | 说明 |
|------|------|------|
| 脚本加载 | 达标 | manifest 中顺序：lib → core（storage/i18n/toc/project）→ adapter → sidebar 子模块 → sidebar → content |
...
| 类别 | 状态 | 说明 |
|------|------|------|
| 权限 | 达标 | manifest 仅 storage + host_permissions（chat.openai.com / chatgpt.com / gemini.google.com） |
...
| DOM 依赖 | 部分达标 | 选择器集中在 chatgpt-adapter / gemini-adapter；README/TEST_REPORT 已说明改版需改选择器 |
...
- ChatGPT / Gemini 页面结构变更会导致选择器失效，需人工跟进并更新 adapter。
...
- **E2E**：若有浏览器自动化环境，可对「加载 → 切换 Tab → 搜索 → 添加项目」等关键路径录一条 E2E。
- **回归清单**：发版前建议使用 docs/CHECKLIST.md 逐项勾选，并更新本报告中的「已知局限」与「结论」日期。
...
| 维度 | 结论 | 备注 |
|------|------|------|
| 一、功能完整性与正确性 | 达标 | 承诺功能已实现，已知局限已文档化 |
| 二、代码质量与可维护性 | 部分达标 | 结构清晰，sidebar.js 体量偏大 |
| 三、用户体验与无障碍 | 达标 | i18n/主题/反馈/无障碍到位 |
| 四、性能与资源 | 达标 | debounce、事件委托、存储使用合理 |
| 五、安全与权限 | 达标 | 最小权限、防 XSS、数据本地 |
| 六、兼容性与健壮性 | 部分达标 | MV3 与目标浏览器达标，依赖目标页 DOM |
| 七、文档与可测试性 | 部分达标 | 文档齐全，无自动化测试 |

**综合**：插件当前状态良好，功能够用、安全与体验达标；改进空间主要在「单文件体量」「自动化测试」与「目标页改版时的可维护性」上。建议按 CHECKLIST.md 做发版前回归，并定期更新本报告。
