...
- [ ] 在任一支持平台会话页（ChatGPT / Gemini / Claude / DeepSeek）右侧出现侧边栏
...
- [ ] 在支持的平台会话页中发 3～5 条用户消息后，「当前对话」中出现对应目录条目
- [ ] 每条目录标题为问题前约 36 字
...
- [ ] 「项目」Tab 中可见各平台 Auto 项目（如 ChatGPT/Gemini/Claude/DeepSeek 中的项目）和「My Projects」
...
- [ ] 在 ChatGPT 会话页侧边栏正常显示且功能可用
- [ ] 在 Gemini 会话页侧边栏正常显示且功能可用
- [ ] 在 Claude 会话页侧边栏正常显示且功能可用
- [ ] 在 DeepSeek 会话页侧边栏正常显示且功能可用（含底部 TOC 摘要、历史/项目中 DeepSeek 图标显示正确）
- [ ] 四平台数据隔离或共享符合预期（当前为同 storage）
