...
describe('StorageManager', () => {
  ...
  it('生产默认不向控制台输出本地存储内容', async () => {
    expect(window.storageManager.DEBUG).toBe(false);
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    await window.storageManager.set({ privateConversation: 'local only' });
    await window.storageManager.get('privateConversation');
